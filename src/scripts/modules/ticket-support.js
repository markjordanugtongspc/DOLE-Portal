...
// Knowledge Base Articles (static content â€” not from DB)
...
class TicketSupportApp {
        ...
        this._globalMessagesChannel = supabase
            ...
            .on('postgres_changes', {
                ...
                if (targetTicket) {
                    ...
                    // New ticket or message for unlisted ticket â€” reload tickets list
                    playNotificationPing();
                    void this.loadTickets();
                }
            })
    ...
    bindChatViewEvents() {
        ...
        // Details panel inputs â€” update ticket in DB on change
    ...
    renderTags(ticket) {
        ...
        tags.forEach((tag, idx) => {
            html += `<span class="inline-flex items-center gap-1 text-[10px] font-bold bg-gray-900 text-white dark:bg-gray-950 dark:text-white px-2 py-0.5 rounded-md">${esc(tag)} <button type="button" class="cursor-pointer text-gray-400 hover:text-white font-bold select-none remove-tag-btn" data-tag-idx="${idx}">Ã—</button></span>`;
        });
                ...
                bubbleContent = `
                    <div class="flex items-start bg-white/10 dark:bg-gray-900/40 rounded-lg p-2.5 max-w-xs border border-white/10 dark:border-gray-700/50">
                        <div class="me-2.5">
                            ...
                            <span class="flex text-[10px] opacity-75 gap-1.5 font-normal">${esc(msg.filePages || '')} â€¢ ${esc(msg.fileSize || '')} â€¢ PDF</span>
                        </div>
