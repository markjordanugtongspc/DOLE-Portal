...
const getGipDirectoryAccount = async (externalUserId) => {
    ...
    if (!directoryUrl || !directorySecret) throw new Error('GIP external directory is not configured.');
    ...
    if (!response.ok || !user?.id) throw new Error('Selected GIP account could not be verified.');
